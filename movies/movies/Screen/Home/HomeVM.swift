...
class HomeVM: ScreenViewModel<GeneralTableView> {
    ...
    func onTap(movie: MovieResults.Result) {
        guard let view = view else { return }
        self.router.routeToMovieDetail(from: view, with: movie)
    }
    ...
    private func setupTapActions() {
        genresCell.onTapGenre = { [weak self] genre in
            ...
            self.router.routeToMovieList(
                ...
                navigationTitle: genre.name.lowercased()
            )
        }
        headlineCell.onTapHeadline = { [weak self] model in
            self?.onTap(movie: model)
        }
        popularCell.onTapMovies = { [weak self] model in
            self?.onTap(movie: model)
        }
        newMovieCell.onTapMovies = { [weak self] model in
            self?.onTap(movie: model)
        }
        comingSoonCell.onTapMovies = { [weak self] model in
            self?.onTap(movie: model)
        }
    }
